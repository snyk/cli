...
const debug = debugLib('snyk-fix:node');

export async function nodeFix(
  entities: EntityToFix[],
  options: FixOptions,
): Promise<FixHandlerResultByPlugin> {
  const spinner = ora({ isSilent: options.quiet, stream: process.stdout });
  const spinnerMessage = 'Looking for supported Node.js items';
  ...
  const results = handlerResult.node;
