...
func initApplicationConfiguration(config configuration.Configuration) {
	config.AddAlternativeKeys(configuration.AUTHENTICATION_TOKEN, []string{"snyk_cfg_api", "api", "snyk_pat"}) // could we just leverage the existing SNYK_TOKEN implementation and add snyk_pat here???
