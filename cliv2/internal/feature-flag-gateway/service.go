...
import (
	...
	"github.com/snyk/go-code-integrations-common/v2/observable_httpretry"
)
...
// endpoint.
//
// Retries are implemented using an exponential backoff strategy.
// Request timeout is based on the historic latency of the endpoint according to
// the metric `p99:http.request.duration.seconds`.
type Client struct {
	baseURL *url.URL
	client  *observable_httpretry.Client
}

// NewService creates a new Flipt client.
func NewService(baseURL *url.URL, requestTimeout, initialBackoff time.Duration, maxAttempts int) (*Client, error) {
	if baseURL == nil {
		return nil, fmt.Errorf("base URL is nil")
	}

	client, err := observable_httpretry.NewClient(maxAttempts, requestTimeout, initialBackoff)
	if err != nil {
		return nil, fmt.Errorf("creating httpretry client: %w", err)
	}
...
// ListFlags lists all flags in a namespace (no pagination).
