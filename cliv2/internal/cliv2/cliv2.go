...
func (c *CLI) Init() (err error) {
	...
	if len(c.CacheDirectory) > 0 {
		...
		if _, err = os.Stat(c.CacheDirectory); os.IsNotExist(err) {
			...
			if err != nil {
				return fmt.Errorf("Cache directory path is invalid: %w", err) //nolint:stylecheck // user-facing error message
			}
		}
	}
...
func (c *CLI) commandVersion(passthroughArgs []string) error {
	if utils.Contains(passthroughArgs, "--json-file-output") {
		return fmt.Errorf("The following option combination is not currently supported: version + json-file-output") //nolint:stylecheck // user-facing error message
	} else {
		c.printVersion()
		return nil
	}
