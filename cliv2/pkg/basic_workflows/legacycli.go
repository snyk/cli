...
func legacycliWorkflow(
	...
	err = ValidateGlibcVersion(debugLogger)
	if err != nil {
		return output, err
	}
...
func ValidateGlibcVersion(debugLogger *zerolog.Logger, opt ...utils.GlibcVersion) error {
	var versionFn utils.GlibcVersion
	if len(opt) > 0 {
		versionFn = opt[0]
	} else {
		versionFn = utils.DefaultGlibcVersion()
	}

	version, err := versionFn()
	if err != nil {
		debugLogger.Error().Err(err).Msg("Failed to get glibc version")
		return nil
	}
	...
	res := utils.SemverCompare(version, minVersion)

	if res < 0 {
		return snyk.NewRequirementsNotMetError(
			fmt.Sprintf("The installed glibc version, %s is not supported. Upgrade to a version of glibc >= %s", version, minVersion),
