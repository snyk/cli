...
try {
  ...
  $existingMvn = Get-ChildItem -Path $installDir -Recurse -Filter 'mvn.cmd' -File -ErrorAction SilentlyContinue | Select-Object -First 1
  if ($existingMvn) {
    Write-Host "[maven-cache] HIT: Maven already extracted at $($existingMvn.FullName); skipping."
    $binPath = Split-Path $existingMvn.FullName -Parent
  }
  else {
  ...
  try {
    ...
    $bashPath = $binPath.Replace('\', '/').Replace('C:', '/c')
    ...
    Write-Host "Warning: failed to persist Maven PATH update to bash env script: $($_.Exception.Message)"
  }
}
catch {
