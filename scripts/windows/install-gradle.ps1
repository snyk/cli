...
try {
  ...
  $existingGradle = Get-ChildItem -Path $installDir -Recurse -Filter 'gradle.bat' -File -ErrorAction SilentlyContinue | Select-Object -First 1
  if ($existingGradle) {
    Write-Host "[gradle-cache] HIT: Gradle already extracted at $($existingGradle.FullName); skipping."
    $binPath = Split-Path $existingGradle.FullName -Parent
  }
  else {
