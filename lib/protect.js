          ...
          return new Promise(function (resolve, reject) {
            ...
            return applyPatch(patch, source, live).then(function () {
              ...
              if (vuln.grouped && vuln.grouped.includes) {
                ...
                var writePromies = [fs.writeFile(flag, now.toJSON(), 'utf8')];
                debug(flag);
                vuln.grouped.includes.forEach(function (id) {
                  var flag = path.resolve(source, '.snyk-' + id + '.flag');
                  debug(flag);
                  writePromies.push(fs.writeFile(flag, now.toJSON(), 'utf8'));
                });
                promise = Promise.all(writePromies);
              } else {
...
function patchError(error, dir, patchFile) {
...
function applyPatch(patch, cwd, live) {
  return new Promise(function (resolve, reject) {
    ...
    exec(cmd + test, {
      ...
      if (error) {
        debug('patch command failed', relative, error.message, stdout.trim());
        return reject(patchError(error, relative, patch));
      }

      if (stdout.trim()) {
        debug('patch failed', stdout.trim());
        return reject(patchError(new Error(stdout.trim()), relative, patch));
      }
