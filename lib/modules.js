...
function loadModulesInternal(root, rootDepType, callback) {
  ...
  var promise = Promise.resolve().then(function () {
    ...
    if (pkg) {
      ...
      console.log('failed to load package on %s', root);

      // set a default value for the pacakge...
      pkg = {};
    }
    ...
    return fs.readdir(path.resolve(root, 'node_modules')).then(function (dirs) {
      ...
      .reduce(function (acc, curr) {
        ...
        if (depType === DEP_TYPE_DEV && !config.devDeps) {
          return acc;
        }
      ...
      var promises = deps.map(function (dep) {
        ...
        return loadModulesInternal(dir, depType);
      });
