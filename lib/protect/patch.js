...
module.exports._getPatchFile = getPatchFile;
...
function getPatchFile(url, filename, rejector, resolver, attempts) {
  request(url)
    .on('end', function () {
      resolver(filename);
    })
    .on('error', function (err) {
      ...
      getPatchFile(url, filename, rejector, resolver, attempts - 1);
    })
      ...
      return acc.then(function (res) {
        ...
        var promises = patches.urls.map(function (url) {
          return new Promise(function (resolve, reject) {
            ...
            getPatchFile(url, filename, reject, resolve, 1);
          }).then(function (patch) {
