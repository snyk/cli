...
function checkModulesInstalled(root, options) {
  var getModulesFromPackage = fs.readFile(path.join(root, options.file))
    .catch(function () {
      throw new Error('Can not read package file!');
    })
    .then(function (contents) {
      var contentsParsed;
      try {
        contentsParsed = JSON.parse(contents);
      } catch (err) {
        throw new Error('Could not parse package.json!');
      }
      if (!contentsParsed.dependencies) {
        return [];
      }
      return Object.keys(contentsParsed.dependencies);
    });
  var getModulesFromDir = fs.readdir(path.join(root, 'node_modules'))
    .catch(function () {
      return [];
    });
  return Promise.all([getModulesFromPackage, getModulesFromDir])
    .then(function (res) {
      var modulesFromPackage = res[0];
      var modulesFromDir = res[1];
      var modulesNonFound = modulesFromPackage.filter(function (item) {
        return modulesFromDir.indexOf(item) === -1;
      });
      if (modulesNonFound.length !== 0) {
        throw new Error('Not all modules from package.json are installed, ' +
          'Please run `npm install` first. Did not find the following ' +
          'modules:\n' + modulesNonFound.join('\n'));
      }
      return true;
    });
}
...
function test(root, options) {
  ...
  var promise = Promise.all([
    ...
    checkModulesInstalled(root, options),
  ]).then(function (res) {
    var exists = res[0];
