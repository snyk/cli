...
function test(root, options) {
  ...
  var promise = Promise.all([
    ...
    fs.readdir(path.join(root, 'node_modules')).catch(function () {return [];}),
  ]).then(function (res) {
    var exists = res[0];
    var nodeModulesExist = res[1];
    var nodeModulesExistSome = res[2];
    ...
    // ToDo: check if all actually used in package.json modules are installed
    if (nodeModulesExistSome.length === 0) {
      // throw a custom error
      throw new Error('Missing modules in node_modules folder: ' +
        'we can\'t test ' +
        'without dependencies.\nPlease run `npm install` first.');
    }
