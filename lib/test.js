        ...
        if (modules && res.vulnerabilities) {
          res.vulnerabilities.forEach(function (vuln) {
            ...
            if (plucked.bundled) {
              ...
              // remove the ability to upgrade this vuln - only required in
              // the first two parts of the `upgradePath`, since first is the
              // root package, and the second is the direct (upgradable) dep.
              [0,1].forEach(function (i) {
                if (vuln.upgradePath[i]) {
                  vuln.upgradePath[i] = false;
                }
              });
            }
          });
