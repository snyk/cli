...
export async function getEnabledFeatureFlags(
  ...
  try {
    const result = await evaluateFeatureFlagsForOrg(flags, orgId);
    const evaluations = result?.data?.attributes?.evaluations ?? [];
    return new Set(
      evaluations.filter((e) => e.value === true).map((e) => e.key),
    );
  } catch {
    return new Set();
  }
