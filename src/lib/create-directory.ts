...
/**
 * Attempts to create a directory and fails quietly if it cannot.
 * Rather than throwing errors it logs them to stderr and returns false.
 * It will attempt to recursively nested directory
 * (ex `mkdir -p` style) if it needs to but will fail to do so with
 * Node < 10 LTS.
 * @param newDirectoryFullPath the full path to a directory to create
 * @returns true if either the directory already exists or it is
 * successful in creating one or false if it fails to create it.
 */
export function createDirectory(newDirectoryFullPath: string): boolean {
  ...
  // if we can't successfully create the directory,
  // either because node < 10 and recursive or some other failure,
