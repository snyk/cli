import { makeAsyncRequest, makeRequest } from './request';
...
async function makeAsyncRequestWrapper(
  payload: any,
  callback?: (err: Error | null, res?, body?) => void,
): Promise<void | { res: needle.NeedleResponse; body: any }> {
  const totalNetworkTimeTimer = MetricsCollector.NETWORK_TIME.createInstance();
...
export {
  makeRequestWrapper as makeRequest,
  makeAsyncRequestWrapper as makeAsyncRequest,
};
