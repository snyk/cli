...
runCmdAsDockerUser "PATH=${PATH} snyk --json ${SNYK_COMMAND} ${SNYK_PARAMS} \
...
if [ -n "$MONITOR" ]; then
  ...
  runCmdAsDockerUser "cat $MONITOR_OUTPUT_FILE \
| jq -r \".uri\" | awk '{print \"<center><a target=\\\"_blank\\\" href=\\\"\" \$0 \"\\\">View On Snyk.io</a></center>\"}' > \"${PROJECT_PATH}/${PROJECT_FOLDER}/${HTML_FILE}\" 2>\"${ERROR_FILE}\""
fi
