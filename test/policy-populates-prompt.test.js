...
test('policy file populates ignore reasons', function (t) {
  ...
  policy.load(__dirname + '/fixtures/colonizers').then(function (config) {
    ...
    var prompt = allPrompts.filter(function (p) {
    ...
    // suffix.
    var vuln = allPrompts.filter(function (p) {
    ...
    t.equal(prompt.default, rule.reason, 'reasons should match in default value');
    t.end();
  }).catch(function (e) {
    console.log(e.stack);
