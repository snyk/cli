...
test('"snyk test --show-vulnerable-paths=false"', function(t) {
  ...
  return cli.test('more-vuln-paths-than-vulns', options)
  .then(function(res) { t.fail('Should have found vulns!'); })
  .catch(function(res) {
...
test('"snyk test"', function(t) {
  return cli.test('more-vuln-paths-than-vulns')
  .then(function() { t.fail('Should have found vulns!'); })
  .catch(function(res) {
    var vulnUrls = res.message
      .match(/^- info: (.*)$/gm)
      .map(function(result) {
