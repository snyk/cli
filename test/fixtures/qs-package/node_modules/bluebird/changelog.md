...
Bugfixes:
 ...
 - Fix .each, .filter, .reduce and .map callbacks being called synchronously if the input is immediate. ([#513](.))
...
Features:
 ...
 - Add new parameter `value` for [`Promise.bind`](API.md#promisebinddynamic-thisarg--dynamic-value---promise)
...
Bugfixes:
 ...
 - Promisified methods from promisifyAll no longer call the original method when it is overridden
