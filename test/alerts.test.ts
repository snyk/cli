...
test('register an alert', (t) => {
  ...
  t.equal(alerts.displayAlerts().trim(), alert.msg, 'alert is displayed');
});
...
test('register two different alerts', (t) => {
  ...
  t.contains(displayedAlerts, alert1.msg, 'first alert is displayed');
  t.contains(displayedAlerts, alert2.msg, 'second alert is displayed');
});
