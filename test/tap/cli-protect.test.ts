...
test('`protect --policy-path`', async (tt) => {
  ...
  tt.test('default policy', async (t) => {
    ...
    const vulns = require(getFixturePath(
      'npm-package-policy/test-graph-result.json',
    ));
  ...
  tt.test('custom policy path', async (t) => {
    ...
    const vulns = require(getFixturePath('npm-package-policy/vulns.json'));
    vulns.policy = expected;
    server.setNextResponse(vulns);
...
test('`protect` with no policy', async (t) => {
  ...
  const projectPolicy = fs
    .readFileSync(getWorkspacePath('npm-with-dep-missing-policy/.snyk'))
    .toString();
