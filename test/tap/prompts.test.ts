...
test('dots in id', async (t) => {
  const prompts = await run(t, getFixturePath('underscore.string.json'));
...
test('review patches', async (t) => {
  const prompts = await run(t, getFixturePath('uglify-patch-only.json'), 2);
...
test('direct update', async (t) => {
  const prompts = await run(t, getFixturePath('hardy.json'), 4);
...
test('direct update post wizard', async (t) => {
  const prompts = await run(t, getFixturePath('hardy-post-wizard.json'), 2);
  t.ok(
    prompts!.some((p) => {
...
test('patches also include (non-working) updates', async (t) => {
  const prompts = await run(t, getFixturePath('uglify-contrived.json'), 2);
...
test('case 0: no remediation', async (t) => {
  const prompts = await run(t, getFixturePath('scenarios/case-0.json'));
  t.ok(contains(prompts![0], 'ignore'));
});

test('case 1: direct update', async (t) => {
  const prompts = await run(t, getFixturePath('scenarios/case-1.json'));
  t.ok(contains(prompts![0], 'update'));
  t.equal(
...
test('case 2: indirect update', async (t) => {
  const prompts = await run(t, getFixturePath('scenarios/case-2.json'));
  t.ok(contains(prompts![0], 'update'));
  t.equal(
    prompts![0].choices[1].value.choice,
    'skip',
...
test('case 4: upgrades to different versions', async (t) => {
  const prompts = await run(t, getFixturePath('scenarios/case-4.json'), 2);
...
test('case 5: two patches modify the same files', async (t) => {
  const prompts = await run(t, getFixturePath('scenarios/case-5.json'), 2);
  t.ok(contains(prompts![0], 'review', true), 'review first');
...
test('humpback - checks related groups and subitems', async (t) => {
  ...
  const prompts = await run(t, getFixturePath('scenarios/humpback.json'), 4);
