...
test('wizard supports review and ignore (SC-943)', async (t) => {
  ...
  const vulns = JSON.parse(
    fs.readFileSync(
      path.resolve(__dirname, './fixtures/scenarios/anna.json'),
      'utf-8',
    ),
  );
...
test('same name vulns do not get ignored (skipping in particular) (SC-1430)', async (t) => {
  ...
  const vulns = JSON.parse(
    fs.readFileSync(
      path.resolve(__dirname, './fixtures/scenarios/SC-1430.json'),
      'utf-8',
    ),
  );
...
test('ignored grouped update explodes into multiple rules (SC-959)', async (t) => {
  ...
  const vulns = JSON.parse(
    fs.readFileSync(
      path.resolve(__dirname, './fixtures/scenarios/explode-ignore.json'),
      'utf-8',
    ),
  );
...
test('patch grouped vuln should run multiple patches (SC-1109)', async (t) => {
  ...
  const vulns = JSON.parse(
    fs.readFileSync(
      path.resolve(__dirname, './fixtures/scenarios/SC-1109.json'),
      'utf-8',
    ),
  );
...
test('vulns from extraneous deps are patched (SC-3560)', async (t) => {
  ...
  const vulns = require(__dirname + '/fixtures/scenarios/SC-3560.json');
...
test('yarn reinstall is not a valid option', async (t) => {
  ...
  const vulns = JSON.parse(
    fs.readFileSync(
      path.resolve(__dirname, './fixtures/oui-wizard-reinstall.json'),
      'utf-8',
    ),
  );
