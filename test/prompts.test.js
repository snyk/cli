...
test('direct update post wizard', function(t) {
  run(t, 2, './fixtures/hardy-post-wizard.json')
    .then(function(prompts) {
      t.ok(
        prompts.some(function(p) {
...
test('humpback - checks related groups and subitems', function(t) {
  ...
  run(t, 4, './fixtures/scenarios/humpback.json')
    .then(function(prompts) {
      var offset = 0;
...
function hasText(question, position, value) {
  ...
  var index = question.choices[position].name
    .toLowerCase()
