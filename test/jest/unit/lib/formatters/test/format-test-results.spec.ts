...
describe('extractDataToSendFromResults', () => {
  ...
  describe('container app and os json results', () => {
    ...
    it('should include OS field with prettyName in JSON output for container tests when OS info is available', () => {
      ...
      expect(parsedJson.os).toBeDefined();
      expect(parsedJson.os.prettyName).toBe('Alpine Linux v3.14');
    ...
    it('should not include OS field when OS info is not available', () => {
      ...
      const parsedJson = JSON.parse(res.stringifiedJsonData);
      expect(parsedJson.os).toBeUndefined();
      expect(parsedJson.ok).toBe(true);
    });
  });
});
