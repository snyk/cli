import { filterDockerFacts, isUnmanagedEcosystem } from '../../../src/lib/ecosystems/common';
import { PluginResponse, ScanResult, Ecosystem } from '../../../src/lib/ecosystems/types';
import { Options } from '../../../src/lib/types';
import * as featureFlags from '../../../src/lib/feature-flags';
import { DOCKER_FACTS_FILTERING_FEATURE_FLAG } from '../../../src/cli/commands/constants';

jest.mock('../../../src/lib/feature-flags');

describe('ecosystems/common', () => {
  describe('isUnmanagedEcosystem', () => {
    it('should return true for cpp ecosystem', () => {
      expect(isUnmanagedEcosystem('cpp')).toBe(true);
    });

    it('should return false for docker ecosystem', () => {
      expect(isUnmanagedEcosystem('docker')).toBe(false);
    });

    it('should return false for code ecosystem', () => {
      expect(isUnmanagedEcosystem('code')).toBe(false);
    });
  });
  ...
  describe('filterDockerFacts', () => {
    ...
    it('should not filter facts for non-docker ecosystems', async () => {
      ...
      const result = await filterDockerFacts(mockPluginResponse, ecosystem, mockOptions);
    ...
    it('should not filter facts when feature flag is enabled (true)', async () => {
      ...
      (featureFlags.hasFeatureFlagOrDefault as jest.Mock).mockResolvedValue(true);
      ...
      expect(result.scanResults[0].identity).toEqual(mockPluginResponse.scanResults[0].identity);
      expect(result.scanResults[0].target).toEqual(mockPluginResponse.scanResults[0].target);
    });

    it('should filter OS project facts (first index) when feature flag is disabled', async () => {
      const ecosystem: Ecosystem = 'docker';
      (featureFlags.hasFeatureFlagOrDefault as jest.Mock).mockResolvedValue(false);
    ...
    it('should filter application project facts (non-first index) with additional filtering', async () => {
      ...
      expect(result.scanResults[0].identity).toEqual(mockPluginResponse.scanResults[0].identity);
      expect(result.scanResults[0].target).toEqual(mockPluginResponse.scanResults[0].target);
      expect(result.scanResults[1].identity).toEqual(mockPluginResponse.scanResults[1].identity);
      expect(result.scanResults[1].target).toEqual(mockPluginResponse.scanResults[1].target);
    });
    ...
    it('should preserve other scanResult properties when filtering', async () => {
      ...
      expect(result.scanResults).toHaveLength(mockPluginResponse.scanResults.length);
    ...
    it('should handle scan results with no filterable facts', async () => {
      ...
      expect(result.scanResults[0].facts).toEqual(mockPluginResponse.scanResults[0].facts);
      // Verify other parts of pluginResponse remain unchanged
      expect(result.scanResults[0].identity).toEqual(mockPluginResponse.scanResults[0].identity);
      expect(result.scanResults[0].target).toEqual(mockPluginResponse.scanResults[0].target);
      expect(Object.keys(result)).toEqual(Object.keys(mockPluginResponse));
    ...
    it('should demonstrate different filtering behavior between OS and application projects', async () => {
      ...
      const app1FactTypes = result.scanResults[1].facts.map(fact => fact.type);
      ...
      const app2FactTypes = result.scanResults[2].facts.map(fact => fact.type);
      ...
      result.scanResults.forEach((scanResult, index) => {
        expect(scanResult.identity).toEqual(mockPluginResponse.scanResults[index].identity);
        expect(scanResult.target).toEqual(mockPluginResponse.scanResults[index].target);
      });
      expect(Object.keys(result)).toEqual(Object.keys(mockPluginResponse));
    });
  });
