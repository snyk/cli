...
describe('snyk sbom monitor (mocked server only)', () => {
  ...
  test('monitor success', async () => {
    ...
    expect(stdout).toContain(`Explore this snapshot at http://example-url/project/project-public-id/history/snapshot-public-id`);
    expect(stdout).toContain(`Notifications about newly disclosed issues related to these dependencies will be emailed to you.`);
  });
  ...
  test('feature flag sbomMonitorBeta is disabled', async () => {
    ...
    expect(stderr).toEqual('');
    expect(stdout).toContain(`The feature you are trying to use is not available for your organization.`);
  });
});
