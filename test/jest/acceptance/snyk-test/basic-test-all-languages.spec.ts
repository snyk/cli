    ...
    describe(`${type} workflow`, () => {
      describe('`snyk test` of basic projects for each language/ecosystem', () => {
        ...
        test.each([
          ...
          'run `snyk test` on a python project $fixture',
          async ({ fixture }) => {
        ...
        test('fails `snyk test` on a python project with wrong command exits with code 2', async () => {
          ...
          await runCommand(wrongPythonCommand, ['--version']).catch(
            function () {
          ...
          const result = await runSnykCLI(
            'test -d --command=' + wrongPythonCommand,
            {
              cwd: project.path(),
              env,
            },
        ...
        test('run `snyk test --maven-aggregate-project` on a maven 4 aggregate project with wrapper', async () => {
          ...
          expect(stdout).toMatch(
            'Project name:      com.example:maven4-aggregate',
          );
          expect(stdout).toMatch(
            'Project name:      com.example:maven4-service',
          );
        ...
        test('run `snyk test` on a composer project', async () => {
          const prerequisite = await runCommand('composer', [
            '--version',
          ]).catch(function () {
            return { code: 1, stderr: '', stdout: '' };
          });
...
=======
  ...
  test.each([
    ...
    async ({ targetFramework }) => {
      ...
      const { code, stderr, stdout } = await runSnykCLI(command, {
        cwd: project.path(),
>>>>>>> 38d071bc0 (test: skip failing windows scenarios)
      });
    });
  },
);
