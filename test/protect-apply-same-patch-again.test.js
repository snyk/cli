...
// fixtures
const fixturesBaseFolder = path.resolve('./test/fixtures/protect-apply-same-patch-again');
...
const alreadyPatchedTransitiveDepFolder = path.resolve(transitiveDepsFolder, 'already_patched_dep');
...
const transitiveDepCodeFilePath = path.resolve(alreadyPatchedTransitiveDepFolder, 'somecode.js');
const transitiveDepCodeContent = 'console.log("some patched code here")';
const transitiveDepBackupFilePath = path.resolve(alreadyPatchedTransitiveDepFolder, 'somecode.js.orig');
const transitiveDepBackupContent = 'console.log("some original code here")';
const transitiveDepPatchFlagPath = path.resolve(alreadyPatchedTransitiveDepFolder, '.some-vuln-id.flag');
const transitiveDepPatchFlagContent = new Date().toISOString();

test('setup', (t) => {
  const fixture = fs.readFileSync(path.resolve(fixturesModuleFolder, 'node-fixture.js'));
...
test('Same patch is applied multiple times without issue', (t) => {
  ...
  return patch(tasks, true)
    .then((res) => {
      ...
      t.ok(fs.existsSync(transitiveDepBackupFilePath), 'backup of transitive dep found');
      t.ok(fs.existsSync(transitiveDepCodeFilePath), 'code of transitive dep found');
      t.ok(fs.existsSync(transitiveDepPatchFlagPath), 'patch flag of transitive dep found');
      t.equal(fs.readFileSync(transitiveDepBackupFilePath).toString(),
        transitiveDepBackupContent, 'backup of transitive dep unaltered');
      t.equal(fs.readFileSync(transitiveDepCodeFilePath).toString(),
        transitiveDepCodeContent, 'code of transitive dep unaltered');
